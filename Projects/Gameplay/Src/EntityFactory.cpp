...
Entity* EntityFactory::createShipEntityClient(EntityCreationPacket p_packet)
{
	...
	Transform* shiptransform = new Transform( p_packet.translation, p_packet.rotation, 
		p_packet.scale);
	entity->addComponent( ComponentType::Transform, shiptransform );
	...
	else
	{
		...
		engineSound->pos = shiptransform->getTranslation();
		soundComponent->addAudioHeader(engineSound);
...
Entity* EntityFactory::createMinigunClient(EntityCreationPacket p_packet)
{
	...
	explodeSound->file = "megafun.wav";//machinegunsoundsnap.wav";//"Spaceship_Weapon_-_Fighter Blaster or Laser-Shot-Mid.wav";
