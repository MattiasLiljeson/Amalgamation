...
public:
	...
	virtual Model* createModelDescription(const string& p_name,
										  const string* p_path=NULL);

	virtual vector<Model*>* createModelDescriptions(const string& p_name,
												    const string* p_path=NULL);
...
protected:
	...
	virtual vector<Model*>* createAllModelData(AglScene* p_scene, 
											   unsigned int p_numberOfModels);

	void readConnectionPoints(unsigned int p_modelNumber, Model* p_model, AglScene* p_scene);

	void readParticleSystems(unsigned int p_modelNumber, Model* p_model, AglScene* p_scene);
