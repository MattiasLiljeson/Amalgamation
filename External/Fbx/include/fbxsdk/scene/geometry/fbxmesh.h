...
public:
	...
	* \return Return the type of this node attribute which is \e EType::eMesh. */
	...
	//@{
		...
		* FBX SDK does not use this information internally but guarantee its persistence in the FBX files and in memory.
		* \param pPolygonIndex Index of the polygon.
		...
		* \remark This value can be smaller than the value returned by GetControlPointsCount() (meaning that
		* not all of the control points stored in the object are used to define the mesh). However,
		* typically, it will be much bigger since any given control point can be used to define a vertex on multiple polygons. */
	...
	* all the methods are implicitly working on Layer 0. Use the \c FbxLayer methods to have access to the other layers. */
	//@{
		...
		* \remark Only the channels that have UVs associated are reported in the array. For example, let's assume that
